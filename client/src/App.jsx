...
function App() {
  ...
  useEffect(() => {
    import('react-hot-toast').then(({ toast }) => {
      toast.success('StartWise AI OS v2.0 Initialized', {
        icon: '🚀',
        duration: 4000
      });
    });
             ...
             <div className="flex flex-col gap-4">
                ...
                {isSidebarOpen && (
                  <div className="flex gap-2">
                    ...
                    <button onClick={logout} className="flex-1 p-2 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-colors flex justify-center"><LogOut size={16} /></button>
                  </div>
                )}
             </div>
          </div>
